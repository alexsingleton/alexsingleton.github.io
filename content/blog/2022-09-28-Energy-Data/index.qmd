...
The purpose of this post is to provide a methodological note about how small area energy related statistics can be derived for Great Britain. This details the code and analytic frameworks needed, and also some of the caveats that need to be associated with the use of specific sources of data.
...
There are a variety of sub national energy estimate data available, and in this study we will just use unit postcode level data, alongside data at Lower Layer Super Output Area / Data Zone (LSOA / DZ). For more details about all of the data available, and methodological detail about how these estimates are created, you can see this [website](https://www.gov.uk/government/collections/sub-national-electricity-consumption-data).

The first data imported are the latest postcode level energy estimates for residential gas and electricity. To enable later matching, all of the spaces were removed from the postcode energy data.
...
Then, the same data for LSOA (England, Wales) and Intermediate Zones (Scotland).
...
Not all of the columns are required in the gas and electricity LSOA / IZ data so these were removed, alongside tidying up the column names to remove spaces.
...
The [Office for National Statistics Postcode Directory (ONSPD)](https://geoportal.statistics.gov.uk/search?collection=Dataset&sort=name&tags=all(PRD_ONSPD%2CAUG_2022)) provides a database of all postcodes in the UK, alongside some descriptive measures and lookup to various nested geographies. To create a more managable dataset, a subset of the ONSPD containing only postcodes within Great Britain and those variables of interest; notably a number of lookups, the IMD and OAC.
...
At the time of putting this analysis together, the latest versions of these data related to 2018. As such, an adjustment factor of 1.05 was applied to the estimimates which relates to the [rate of inflation to 2021 prices](https://www.bankofengland.co.uk/monetary-policy/inflation/inflation-calculator) from 2018. For the purpose of these analysis we are also ignoring the confidence intervals of the estimates; which are available for the England and Wales data. There are further differences in that the England and Wales data are presented for average (mean) household disposable (net) income; whereas in Scotland these are for gross income, which might also include welfare benefits, tax credits and housing benefit.

A combined lookup is created containing both LSOA / DZ. Because the annual income estimates for England and Wales are available at MSOA, the analysis is assuming these values to be constant for all nested LSOA within the MSOA.
...
The Unique Property Reference Number (UPRN) data are [downloaded](https://geoportal.statistics.gov.uk/datasets/ons-uprn-directory-august-2022/about) and imported through the following code. The folders are unzipped and placed in the working directory. You won't find these on the [github repo](https://github.com/alexsingleton/energy_map) as they are very large. Because the data are so large, an R interface to Apache Arrow was used to create parquet files from all the extracted CSV data and then unify these into a single file.
...
# This this code only worked on a 128 gig memory machine - can be used instead of the above steps; which are two step - create then read parquet.
uprn <- open_dataset(paste0("./Data/",files),
   delimiter = ",",
   schema = sch,
   skip = 1
...
A number of polygon files are needed to create the later maps, however, direct links were not available for England and Wales so were downloaded manually and loaded into R locally. These included:
...
Energy Performance Certificate (EPC) data are available to download in bulk from this Department for Leveling Up, Housing & Communities [website](https://epc.opendatacommunities.org/domestic/search). This requires registration before download, and the extracted data are quite large (33 gig of unzipped .CSV files). For more information about EPC, the [Energy Saving Trust](https://energysavingtrust.org.uk/advice/guide-to-energy-performance-certificates-epcs/) website provides some useful information. The certificates are valid for 10 years and mainly assigned to properties as part of the process of property purchase or rental. Bands run from A (better) to G (worse), based on the energy performance of the property. There area a huge range of different attributes within these data, however not all are needed for these purposes so are cut down.

After download, the data was manually unzipped into a folder called "all-domestic-certificates" in the root of the analysis folder.
...
After the dataset was created, postcodes were used to append the attributes of the onspd file. The date of the certificated lodgement was also recorded, and a new variable to identify property with certificates within the highest rated bands of A,B and C.
...
latest_epc_national <- bands %>%
    group_by(UPRN) %>%
    slice(which.max(LODGEMENT_DATE2)) %>%
    filter(!is.na(UPRN)) %>% 
    as_tibble()
...
The proportion of EPC assesments with an A,B or C outcome by UPRN can then be mapped over time. The national proportions for England and Wales are shown in green, and the selected area in orange. The red line indicates the ten year cutoff for certificates expiring.
...
epc_change_graph %>%
  ...
  labs(x="Year",y="%",
               title="Percentage of Addresses Assessed with an EPC of A,B or C") 

```

## Calculating the proportion of UPRN with an Energy Certificate

To gauge the completeness of certificates, a count of UPRN that are not ascribed a large user postcode are compared to the number of UPRN that have been attributed an EPC within the past 10 years (period of certificate validity).
...
In `paste0(wpc_map)` there are `r bands %>% filter(pcon == wpc_map, is.na(UPRN)) %>% count(UPRN) %>% select(n) %>% pull()` assessments without a UPRN, however, for the `r nrow(EPC_WPC)` remaining assessments with a UPRN over the past 10 years, the distribution of their most current assessment is as follows:
...
ggplot(aes(CURRENT_ENERGY_RATING)) +
  ...
  scale_y_continuous(labels=percent_format()) +
  labs(x="EPC Current Rating",y="%", title="Certificate Distribution of Addresses Assessed") +
...
We can now calculate the proportion of properties with and EPC assessment.
...
ggplot(df, aes(fill = "#4AB19D", ymax = percentage, ymin = 0, xmax = 2, xmin = 1)) +
 geom_rect(aes(ymax=1, ymin=0, xmax=2, xmin=1), fill ="#4AB19D") +
 geom_rect() + 
 coord_polar(theta = "y",start=-pi/2) + xlim(c(0, 2)) + ylim(c(0,2)) +
 geom_text(aes(x = 0, y = 0, label = paste0(round(tot_uprn_wpc / tot_uprn * 100),"%"), colour="#344E5C"), size=12, family="Poppins SemiBold") +
...
The first step is to append attribute data to the LSOA / DZ simple feature collection that will be used to generate different measures and maps:
...
The following represent derived variables to map.
...
Because LSOA / DZ do not perfectly nest within Westminster Parliamentary Constituency, it is first necessary to explore these intersections and create a lookup table.
...
## Estimated Residential Electricity Cost Map
...
## Estimated Residential Gas Cost Map
...
`r if(fp_N > 0 ) {"\\begin{comment}"}`

There are an estimated `r paste0(sum(fp$total_population_PCD))` people who spend more than 10% of their income on fuel, with `r paste0(sum(fp$population_aged_65_and_over_PCD))` of these being over 65 years of age.

`r if(fp_N > 0 ){paste0("A file containing the postcodes within this area can be downloaded [here](",wpc_map,".csv)")}`.

`r if(fp_N > 0) {"\\end{comment}"}`
